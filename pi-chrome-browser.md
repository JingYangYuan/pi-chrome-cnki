> **公开文档副本**：本文件由 [`paper-master-4ss`](https://github.com/JingYangYuan/paper-master-4ss/blob/main/scripts/export_pi_chrome_doc.py) 的 `export_pi_chrome_doc.py` 从 `modules/lit/references/pi-chrome-browser.md` 导出，**请勿直接编辑本仓库**；修改请改总控源文件后重导。
> 完整 CNKI 闭环协议（检索式、点击判据、验证码几何判据、下载与归档）见 [paper-master-4ss](https://github.com/JingYangYuan/paper-master-4ss) 的 lit 模块。
